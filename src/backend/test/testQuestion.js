...
describe('POST /api/quizzes/:quizId/question', function() {
  ...
  it('verify that quiz has id of newly created question', function(done) {
    request(app)
      ...
      .end((err, rsp) => {
        if (err) throw err;
        const questionDetails = rsp.body;
        ...
        if (!questionIdExists) {
          return done(new Error('POST /api/quizzes/:quizId/question does not add question id to list if id'));
        }
        return done();
      });
  });
  ...
  it('removes the id removed question in quiz`s answers[] array', function(done) {
    request(app)
      ...
      .end((err, rsp) => {
        ...
        if (!questionIdExists) {
          return done(new Error('POST /api/quizzes/:quizId/question does not remove a question id there'));
        }
        return done();
      });
  });
