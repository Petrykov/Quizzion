  ...
  axios.get(config.baseURL + '/v5/var')
    .then((response) => {
      ...
      for (let i = 0; i < elements.length; i++) {
        try {
          const answer = JSON.parse(elements[i].label);
...
router.get('/answer/:answer_id', (req, rsp) => {
  axios.get(config.baseURL + '/v5/var/' + req.params.answer_id)
    .then((response) => {
      ...
      try {
        const answer = JSON.parse(element.label);
        rsp.status(200).json({
          id: element.vh,
          name: element.name,
          label: answer.label,
...
router.post('/answer', (req, rsp) => {
  ...
  const result = bodyCheck.checkPOSTanswer(req.body);
  if (result.length !== 0) {
...
router.put('/answer/:answer_id', (req, rsp) => {
  ...
  async function continu() {
    ...
    const answer = response.data.var[0];
